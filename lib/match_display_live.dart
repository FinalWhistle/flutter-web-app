...
class MatchDisplayLive extends StatelessWidget {
  final Match match;

  const MatchDisplayLive(this.match, {super.key});
  ...
  /// Widget to determine and display the live score based on sport
  Widget getScore() {
    String displayText;

    if (match.sportSlug == 'gaelic' ||
  ...
  /// Widget to display live badge with clockMessage or currentMinutes
  Widget liveBadge() {
    String badgeText;

    final clockMessage = sanitizeText(match.clockMessage); // Sanitize clockMessage
    final currentMinutes = match.currentMinutes?.toString() ?? ''; // Ensure string

    if (clockMessage.isNotEmpty) {
      badgeText = clockMessage; // Display clockMessage if available
    } else if (currentMinutes.contains('+')) {
      badgeText = 'Added Time'; // Handle added time
    } else if (currentMinutes.isNotEmpty) {
      badgeText = '$currentMinutes\''; // Add minute suffix
    } else {
      badgeText = 'LIVE';
    }
    ...
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFF0076A3), // Brand color
